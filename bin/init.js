...
function sqsrch(e){if(e==0){e='it_doa'; e=document.getElementById(e).value; e=dtconv(e);}if(e==1){e='it_vehicleno'; e=document.getElementById(e).value.trim();}	if(e!=''){sqsrchit(e);}}
function sqsrchit(v){document.getElementById('srch').value=v; document.getElementById('userSearch').submit(); submitSearch();}
function dtconv(f){function pad(s){return (s < 10) ? '0'+s:s;} var d=new Date(f); return [pad(d.getDate()), pad(d.getMonth()+1), d.getFullYear()].join('.');}
/*adding item*/
...
function chkAvl(ele){if(ele.options[ele.selectedIndex].text=='+ ADD NEW'){var flnd=prompt('Enter new name:'); if(flnd){_ldgAct(40); dataLoad('data','auth=4&s&type='+ele.id+'&v='+flnd); var lcc=document.createElement("option"); lcc.setAttribute('value',flnd); lcc.setAttribute('selected','true'); lcc.innerHTML=flnd; ele.appendChild(lcc);}else{ele.selectedIndex=0;}}}
