...
function opt(i){
	...
	if(i==4){window.open('view/?report&0&v');}
	if(i==5){window.open('view/?export&0&v');}
	if(i==7){_ldgAct(90); dataLoad('signout','auth');}
}
function optCl(){_menuHide(true); document.getElementById('menuBtn').classList.add('sh');}
...
function viwItem(id){window.open('view/?'+app_cs+'&'+id+'&v');}
/*adding item*/
function addNewTgl(){
	var b=document.getElementById('addNewBtn');
	if(b.value!='- CANCEL'){if(document.getElementById('des').innerHTML==''){b.value='...'; _ldgAct(60); dataLoad('data','auth=3&s');}else addNewTglDone();}
	else{b.value='+ ADD'; document.getElementById('des').classList.add('des'); b.blur();}
}
...
function userSubmit(){
	if(!usrSubmitMode){var c=confirm('Are you sure that you want to ADD this item?'); if(c){_ldgAct(80); usrSubmitMode=true;} else return false;}
	else return false;
}